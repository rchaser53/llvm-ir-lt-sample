...
macro_rules! c_string {
  ($w:expr) => ( CString::new($w).unwrap().as_ptr() );
}
...
pub fn loop_struct(
  ...
  unsafe {
    ...
    let mut struct_test_func_types = [
      LLVMPointerType(named, 0),
      LLVMInt32Type()
    ];
    let func_type = LLVMFunctionType(LLVMInt32Type(), struct_test_func_types.as_mut_ptr(), 2, 0);
    ...
    set_field_value(lb.builder, target_struct, 0, LLVMConstInt(LLVMInt32Type(), 12, 0));
    let mut field = get_field_value(lb.builder, target_struct, 0);

    let struct_test_func_ret = LLVMBuildAdd(lb.builder, field, LLVMGetParam(struct_test_func, 1), c_string!(""));
    ...
    let mut loop_end_block = LLVMAppendBasicBlockInContext(lb.context, main, c_string!("loop_end"));
    ...
    let mut lhs = LLVMBuildAdd(lb.builder, LLVMConstInt(LLVMInt32Type(), 1, 0), load_val, c_string!("a.b"));
    ...
    let mut for_cond = LLVMBuildICmp(lb.builder, LLVMIntPredicate::LLVMIntUGT, lhs, LLVMConstInt(LLVMInt32Type(), 2, 0), c_string!(""));

    LLVMBuildCall(lb.builder, llvm_printf_int, printf_args.as_mut_ptr(), 2, c_string!(""));
